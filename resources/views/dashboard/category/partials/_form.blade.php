@php
    ...
    $cat_type = isset($form_data) ?  $form_data->cat_type : old("cat_type");
    $main_cat_id = isset($form_data) ?  $form_data->main_cat_id : old("main_cat_id");
...
<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
    <div class="form-group">
        <label for="name_input">@lang('site.main_cat_id')</label>
        <select name="main_cat_id" class="form-control" id="main_cat_id">
            <option value="">@lang('site.select')</option>
            @foreach($data['main_categories'] as $t)
                <option {{ $main_cat_id == $t->id ? "selected" : "" }} value="{{ $t->id }}">
                    {{ $t->getTranslateName(app()->getLocale()) }}
                </option>
            @endforeach
        </select>
        <span class="form_error" id="main_cat_id_error"></span>
    </div>
</div>
