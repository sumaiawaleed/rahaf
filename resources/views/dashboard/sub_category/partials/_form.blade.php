@php
    ...
    $cat_type = isset($form_data) ?  $form_data->cat_id : old("cat_type");
...
<div class="col-xl-6 col-lg-6 col-md-6 col-sm-6 col-12">
    <div class="form-group">
        ...
        <select name="cat_type" class="form-control selectpicker" data-live-search="true">
            <option selected>@lang('site.select')</option>
            @foreach( $data['categories'] as $m)
                <option {{ $cat_type == $m->id ? "selected" : "" }} value="{{ $m->id }}">{{ $m->getTranslateName(app()->getLocale()) }}</option>
            @endforeach
        </select>
        <span class="form_error" id="cat_type_error"></span>
    </div>
</div>
